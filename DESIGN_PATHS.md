...
| E     | `design-e`            | `design-e`               | https://v10c-design-e.web.app          | **Vercel / Geist.** Tokeny nejsou odhadnuté — jsou to doslovné `--ds-*` hodnoty z Vercelího balíčku `@vercel/geistdocs` (theme.css 1.19.5), proto v `oklch`. Písmo Geist Sans + Geist Mono (npm `geist` 1.7.2, SIL OFL, soubory v `public/fonts/`). Achromatické rozhraní s modrou jako jedinou interaktivní barvou, hranu kreslí `box-shadow 0 0 0 1px` místo `border`, radius 6 px, primární tlačítko se převrací (černé / bílé), tmavý režim je ČISTĚ ČERNÝ. **Strukturálně:** žádný levý panel — navigaci nese vodorovná hlavička ve dvou řadách (cesta přes lomítka `značka / organizace / kde jsem`, pod ní záložky s podtrženou aktivní), obsah v 1200px sloupci, nadpis stránky stojí přímo na ploše místo v kartě. |
