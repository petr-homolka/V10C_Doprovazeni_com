      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map((ref) => (
            <span
              key={`${ref.kind}-${ref.id}`}
              className="inline-flex items-center gap-1 rounded-full border border-border-strong bg-surface-soft px-2.5 py-1 text-xs text-text-primary"
            >
              {labelFor.get(`${ref.kind}:${ref.id}`) ?? ref.id}
              <button
                type="button"
                onClick={() => handleRemove(ref.kind, ref.id)}
                aria-label={`Odebrat ${labelFor.get(`${ref.kind}:${ref.id}`) ?? ref.id}`}
